# This is Probably the Most Comprehensive, Readable, and Ea...

- BestBlogs URL: https://www.bestblogs.dev/en/article/c39db2ba
- Extraction: DOM text from BestBlogs page via OpenCLI browser bridge
- Extracted chars: 32838
- Original publisher URL: https://mp.weixin.qq.com/s?__biz=MjM5ODYwMjI2MA==&mid=2649798683&idx=1&sn=8319ed2ef27f74711c611c72f7a0e2ec
...
可对于，万亿的训练数据，人工标注已经成了不可能。人们希望能够自动的去选取“目标词”进行监督，怎么做？人们会把一个句子或者一个段落中的分词，进行随机掩盖，然后交给大模型去进行概率预测，而预测的目标值就等于被掩盖的分词，整个过程，不需要人工标注，主要依赖大模型自身的随机掩盖，随机预测。例如让模型学习古诗时，输入“青海长云暗雪山，孤城遥望玉门关”，如果把“玉门关”进行遮盖后，“玉门关”便作为我们本次训练的目标分词。因此我更愿意把“监督”解释为“人工标注”。
...
前文提到，概率是通过归一化函数-softmax产生的，人们在softmax公式上“稍作手脚”后，便实现了上面的热胀冷缩现象。这里做手脚的方式就是在公式中增加了一个整数变量，我们把这个变量，成为温度系数。(上图右侧为使用DeepSeek时官网推荐的温度系数)

听起来温度系数与大模型的训练毫不相干，事实上，它与接下来即将登场的知识蒸馏密不可分，而知识蒸馏是一种小模型训练的最重要的方式之一。

7.9.蒸馏学习

温度系数中的温度，随着度数的升高，液体转化成气体，气体在冷凝管中，遇冷再次凝结成没有杂质的水，这就是蒸馏的过程。我们家里烧水做饭，锅盖上挂着的小水珠也是这样的过程。模型中的蒸馏学习，指的就是要学习蒸馏出来的部分，而不去学习没有剩下的“杂质”。我相信发明这个名词的人，想必对化学或者物理也特别地感兴趣，我查了下才发现，原来这个人叫辛顿（2024年，因为在神经网络突出贡献获得诺贝尔物理学奖）。

学习，肯定是要向别人学习，通常的叫法是，学生模型向老师模型学习。老师模型一般是预训练好的大模型，参数量极大，训练和推理用到的算力成本较高，不太适合轻量化部署，比如部署到你的PC，甚至是你的手机上。而学生模型恰恰就是为了填补老师模型的这个空白，它的初衷就是用尽可能少的参数，来达到向大模型的能力。因此这种学习或者训练的方式，也被叫作模型压缩。那么蒸馏学习的过程是怎么样的？蒸馏是蒸馏的什么？

如果下图所示，虽然，大模型是通过概率预测的方式，来选择一个概率最高的下一个目标词。但有时，我们可以通过控制温度系数，让概率没那么高但词义相近的分词被选中（前文已经解释过）。这对于模型来说，也是有用的知识，而这种知识常常被称为隐藏知识。隐藏知识需要通过提高温度系数来挖掘。而蒸馏学习的目的就是既要学习最高概率的分词，也要尽可能学习这些隐藏知识。例如，在图中，虽然我们习惯说“我想吃饭”，但是我也偶尔会说“我想干饭”，极少数情况，我也喜欢别人对我“请您用膳”。这些对于大模型来说，都是有价值的知识，虽然这几个词本身概率没有“吃饭”高。
...
教师模型提高温度系数，得到的概率分布，被称为软目标，同样，在学生模型中，用同样温度系数计算得到的概率分布，被称为软预测。二者之间通过损失函数计算的误差，被称为蒸馏损失，或者软损失。
...
在过去的一段历史时间中，区块链、显卡也火爆一时，在区块链中，比拼的是谁能以最快的速度挖到下一个区块，这个“挖”的过程实际上，仅仅就是，基于交易信息来计算hash值，谁计算出的hash值的某几位等于预设好的目标值，那么，谁就挖矿成功了。这个过程也叫hash碰撞。比如下一区块的hash值，前4位必须等于0x0700，谁第一个计算出来，谁就挖矿成功，并获得挖矿奖励。而大模型的训练过程，通过训练数据，不断调整参数，谁调整出的参数接近目标值，谁的模型就是好的。说白了，这也是一种“碰撞”，这是它们的异曲同工之处。
...
一块CPU有4核心、16核心、32核、100多核也就差不多了，但你敢想象吗？一块英伟达GPU H100卡就有18000多核。这些核被称为CUDA核(图中INT32\FP32就是一个CUDA核，分别表示计算整型、浮点数的核心,引自英伟达官网)，虽然它们数量多，但是仅仅能进行简单的加减乘除计算，不过这对于AI的训练和推理已经足够了。因为大模型的训练和推理的过程，也不过就是简单的加减乘除。

但很多看似简单的东西，其实并不平凡，如何把AI训练中涉及到的简单矩阵运算，均匀的分摊给这么成千上万的core进行并行计算，这就是很大考验了。而英伟达的CUDA平台就提供了一站式这样的均衡调度能力。

8.3.大模型并行计算

CUDA平台要考虑如何把给它的“东西”分发给成千上万的GPU内核，而大模型并行训练时，要考虑以什么维度去并行计算。例如，在网络负载均衡器上，可以按照五元组加权轮询算法分担流量给到不同后端，也可以按照最小连接数算法，将当前连接分发给连接数最小的后端。

如果你耐着性子，看完前面的章节，我相信你一定对参数和训练数据并不陌生。因为模型参数决定了模型的下限，而训练数据往往能够决定它的上限（仁者见仁，智者见智）。AI训练的并行方式，也是通过这两个维度进行的划分。一类是模型并行；一类就是数据并行。由于模型并行，等同于把权重等参数分布在不同的GPU上进行计算，而这些参数本身是由张量来表示的，因此模型并行也称张量并行。
...
假设有两份训练数据，分布在了两块GPU卡上，每块GPU卡上，通过反向传播算法，各自计算出所有参数的梯度（回忆一下，梯度就是参数的更新量）。关键的时刻来了：此时每个GPU卡上梯度会被传递到中心服务器上，由中心服务器进行梯度聚合（可以用Reduce表示聚合）。聚合的方式就是求和再取平均值。然后中心服务器再把平均梯度广播到所有GPU卡上，统一更新参数。这个中心服务器被称为参数服务器。

但是因为参数服务器会成为通信的瓶颈，现代AI训练大多摒弃了中心化的参数服务器。而是通过更优的连接结构和算法，比如环行结构、树形结构等(受限于笔者水平和写作目的不展开，但不是全互联通信方式)，这使得所有GPU都能得到所有的梯度值，这样的话，每块GPU都要进行Reduce求和平均计算，因此这种通信方式被统称为AllReduce。
...
由于大模型牵涉的都是深度神经网络，牵涉的矩阵运算很多，我们需要把上一层的矩阵运算结果像前逐步计算传递。因此就需要把上一步每块GPU计算的结果传递到负责下一层计算的GPU上进行拼接，以获得全局的计算结果。拼接的过程不涉及计算，或者说，不涉及原有数据的增加或者减少，这与Reduce完全不同。因此人们为拼接又取了一个新名字，叫Gather。如果所有的GPU都需要获取这样的拼接数据，那么整个过程也被称为AllGather。
...
单块 H100 GPU 拥有 18000 多个计算核心，而一套 H100 整机可集成 8 块这样的 GPU。每两块 GPU 之间，都能通过带宽高达 900GB/s 的高速通道互联 —— 这条通道由 18 条链路构成，每条链路的双向总带宽为 50GB/s，这类专用高速链路就是NVLink。

值得注意的是，GPU 之间并非直接互联，而是各自接入平台内部的专用交换机，即NVSwitch，一套 H100 整机中通常配置 4 台 NVSwitch。
...
网络管理员们会把光缆一端接在GPU服务器的网卡上，另一端接在交换机上，这类交换机直接与服务器打交道，我们称为接入交换机（也成leaf）。值得注意的是，每个网卡上联不一样的leaf，这样的话，就需要8台leaf交换机。这样的做的目的，一方面通过多平面设计实现容灾；另一方面是可以让单台交换机完成不同服务器上相同编号的GPU通信，让网络通信变得更有确定性。
