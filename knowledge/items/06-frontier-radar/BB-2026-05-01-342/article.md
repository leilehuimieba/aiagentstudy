...
- BestBlogs URL: https://www.bestblogs.dev/article/452481cb
- Original publisher URL: https://huggingface.co/blog/delta-weight-sync
...
hf upload $USER/vllm-wordle-inference examples/scripts/openenv/vllm_space/ --type space
  ...
  Take Llama-3.1-405B. In bf16 that is 810 GB on disk. PULSE measures
   99% mean per-step sparsity at RL learning rates, so the actual delta sits around 1% of the parameters. Their deployment-measured encoding hits 108 MB on a 7B model, which is the **
   130×** reduction PULSE reports. Scaled linearly to 405B, the delta lands at roughly 6 GB per step.
