# HorizonVault Deep Dive: Achieving 100GB/s+ Distributed Storage Throughput on HDDs | Dewu Tech

- BestBlogs URL: https://www.bestblogs.dev/article/897d1f95
- Original publisher URL: https://mp.weixin.qq.com/s?__biz=MzkxNTE3ODU0NA==&mid=2247544708&idx=1&sn=cc8d064185a5bdc27fd7748f0958182d
- Source: BestBlogs / 得物技术
- Publish time: 2026-05-27 18:30:00
...
目录
  ...
  x 路由和隔离机制带来的有效利用率
    ...
    这几个因子里，最基础的是单盘顺序写。HDD 不适合大量随机小写，但持续顺序写仍然有不错的吞吐。HorizonVault 的 Store 写入路径尽量保持单调：真实数据进入 LogSegment，定位信息进入 IndexSegment，不在主写路径上做复杂原地更新，也不把数据和索引混在一起反复改写。
  ...
  <partitionId>/
    ...
    这几个目录各有分工。LogSegment 承载真实 record，是吞吐主路径；IndexSegment 保存 StoreKey 到 log segment 位置的映射，是读路径定位的基础；ChangeLog 记录删除和变更，辅助恢复和回放；replica.meta 持久化副本本地状态。
