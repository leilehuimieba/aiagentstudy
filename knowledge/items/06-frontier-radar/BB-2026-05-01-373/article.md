# Introducing DSA Attention to Multimodal Models: Kuaishou Keye 2.0 Opens a New Paradigm for Enhanced Reasoning

- BestBlogs URL: https://www.bestblogs.dev/article/1a4247e6
- Original publisher URL: https://mp.weixin.qq.com/s?__biz=Mzg2NzU4MDM0MQ==&mid=2247500530&idx=1&sn=246a6c7439b7dfd5d31dccd574cd22ff
- Source: BestBlogs / 快手技术
- Publish time: 2026-05-26 18:37:00
...
当你把一段长达9分钟、在“晴空万里”与“冰天雪地”间剧烈切换的冰岛旅行Vlog输入给大模型，并要求它做一份旅行攻略时，常规的视觉大模型通常只能给出一份基于字幕和画面标签拼凑的“流水账”。
  ...
  需要说明的是，该榜单论文官方仅公布了Gemini-2.5-Pro的成绩，为了保证评测的严谨性，快手团队严格按照相同评测方式，在内部同步测试了Gemini 3 Flash与Gemini 3.1 Pro作为实测基线比对：

  
   
    日常动作时序解析（Charades-TimeLens）：模型以58.27的mIoU强势对标实测的最强视频理解闭源标杆Gemini 3 Flash（61.19）与Gemini 3.1 Pro（55.71）。

   

   
    视频动作定位（ActivityNet-TimeLens）： 凭借58.54的mIoU，实现了对官方数据Gemini-2.5-Pro（58.1）以及实测数据Gemini 3 Flash（56.95）、Gemini 3.1 Pro（55.08）的全面超越。

   

   
    高光时刻提取（QVHighlights-TimeLens）：模型的mIoU达到69.92，与官方榜单的顶尖闭源模型并驾齐驱，并大幅超过实测的Gemini 3 Flash（49.45）与Gemini 3.1 Pro（46.09）。
  ...
  得益于MOPD技术的有机融合，模型不仅在Video和Agent等核心阵地登顶，更在极度考验逻辑的数学推理（Math & Reasoning）、STEM以及指令遵循（Instruction Following）等通用能力上迎来了全面爆发。
   ...
   5. Context-RL与严苛数据引擎，构筑推理的绝对可靠性
