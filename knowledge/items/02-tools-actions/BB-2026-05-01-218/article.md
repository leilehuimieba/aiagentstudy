# bili-fe-workflow — Practices in Commercial Intelligent Development Workflow

- BestBlogs URL: https://www.bestblogs.dev/en/article/12477550
- Extraction: BestBlogs API proxy content endpoint + rendered rich text body
- Extracted chars: 10057
- Original publisher URL: https://mp.weixin.qq.com/s?__biz=Mzg3Njc0NTgwMg==&mid=2247504231&idx=1&sn=bea903410681034074d405e4c8ac6aa2
...
prd-preprocess 命令:
│
├─→ 读取 knowledge/business.md
│ └─→ 理解业务模块划分，识别涉及模块
│
├─→ 读取 knowledge/components.md
│ └─→ 了解现有组件，避免重复开发
│
├─→ 读取 rules/tech-stack.md
│ └─→ 确认技术栈约束
│
└─→ 输出: 结构化 PRD + 技术可行性分析
...
整理澄清内容，生成 https://clarification.md 文档，并且拆分子需求。就像我们自己开发的时候，也不会一次性就把整个需求写完，而是会先写一块，再写另一块。对于AI来说也是一样，由于上下文的限制AI也不能一次性就将整个需求写完，通过将一整个大需求拆分成几个小型需求，能够让 AI 生成的更加准确。这也属于 模仿和拆解 的思路。
...
这个阶段的产出是两样东西：可运行的 UI 代码和一份 logic-hints.md。UI 已经还原，但按钮点击事件是空的、接口调用是 mock 的、表单校验还没写——这些都被精确地标记在 logic-hints.md 中。
...
├── manifest.md # 步骤5 确认后生成（防止上下文压缩导致丢失）
├── raw/ # 原始 Figma JSON（完整备份，用于追溯）
├── extracted/ # 提取后的结构化数据（代码生成时按需读取）
│ ├── code.html, styles.json, components.json
│ ├── assets.json, tw-to-css.json
└── screenshots/
...
Dev 根据logic-hints.md 内容进入逻辑补全模式，各个Skill的行为调整如下：
