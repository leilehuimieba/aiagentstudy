...
- BestBlogs URL: https://www.bestblogs.dev/article/9a749c49
- Original publisher URL: https://www.freecodecamp.org/news/eks-cost-optimization-reduce-your-aws-bill-using-karpenter-and-rightsizing/
- Source: BestBlogs / freeCodeCamp
- Publish time: 2026-06-23 00:38:45
...
cd eks-cost-optimization
  ...
  Here's what a typical $85,000/month EKS cluster looks like when you break it down:
     ...
     $52,000
     61%
     High — over-provisioning, wrong instance types
     ...
     $15,300
     18%
     Very High — cross-AZ and NAT Gateway charges
     ...
     $10,200
     12%
     Medium — unattached volumes and gp2 vs gp3
     ...
     $4,250
     5%
     Low to Medium — single-service ALBs
...
aws ec2 describe-snapshots \
  ...
  6.3 The ROI of EBS Optimisation
     ...
     Unattached volumes removed (50 × 100GB)
     $500
     $0
     $500
...
kubectl get ingress shared-ingress -n production -w
  ...
  The cost difference:
     ...
     LoadBalancer Service per microservice (20 services)
     20 ALBs
     ~$400/month
  ...
  The Complete 7-Step Sequence
     ...
     1
     Right-size pod resource requests (VPA)
     1 week
     $15,600
     ...
     2
     Install Karpenter with consolidation
     1 week
     $8,400
     ...
     4
     Migrate compatible workloads to Graviton
     2 weeks
     $7,600
     ...
     5
     Add VPC endpoints for S3, ECR, DynamoDB
     1 day
     $6,471
     ...
     6
     Migrate gp2 to gp3 and delete orphaned volumes
     1 day
     $555
