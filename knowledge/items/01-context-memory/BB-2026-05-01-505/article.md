# SQL Compliance Rate Raised to 95%: A Full Analysis of De Wu Data Warehouse's Harness Practice

- BestBlogs URL: https://www.bestblogs.dev/article/f747d09f
- Original publisher URL: https://mp.weixin.qq.com/s?__biz=MzkzMjYzNjkzNw==&mid=2247636546&idx=1&sn=f7a8e28c85273f1986ed216404396df0
- Source: BestBlogs / dbaplus社群
- Publish time: 2026-06-04 07:15:00
...
fiif [ ${
#ERRORS
[@]} -gt 0 ]; thenecho "=== SQL 规范检查失败：$FILE_PATH ===" >&2
...
输出 DQC 配置 JSON，可直接使用
    ...
    以 /dw-etl 为例：一条命令封装了什么？ETL 开发（Step 3）是 Harness 工程价值最高的步骤，SKILL 文件内封装了：

    ① 规范内容（写死，不靠记忆）：

    建表规范：分区字段必须是 partition_dt STRING（格式 YYYYMMDD）；金额字段：DECIMAL(26,4)，不用 DOUBLE；INSERT 模式：必须使用 INSERT OVERWRITE，必须带 PARTITION 子句；禁止：SELECT *、跨库 JOIN（非血缘关系）。
...
└── ddl_sr_[表名].sql   ← SR 建表语句（Key 列顺序、DECIMAL 精度）
    ...
    每次 .sql 文件写入后，PostToolUse hook 自动执行 validate_sql.sh；发现 SELECT * 或缺少 PARTITION → exit 2 阻断，Claude 强制修正。

    ④ subagent 卸载（防 context 膨胀）：

    "生成完毕后，用 sql-validator subagent 验证两个文件"；验证结果全在 subagent context 里，主对话只收到"PASS/FAIL + 问题列表"。
    ...
    项目级上下文持久化。在数仓项目目录下创建 .claude/CLAUDE.md，写入当前迭代状态。每次新迭代开始时更新"正在开发"和"本次迭代约束"两节，上线后清空约束节。全局规范永久保留，控制在 100 行以内。
    ...
    配置 hooks 自动验证。创建 .claude/settings.json + hooks/ 目录，配置 PostToolUse hook（每次写 .sql 后自动规范检查）和 PreToolUse hook（拦截危险 DDL）。效果：不需要每次手动说"帮我检查 SQL 规范"，hook 在 Harness 层自动触发，不占用 Claude 推理资源。
    ...
    创建 subagents 隔离高 token 操作。创建三个核心 subagent 文件：sql-validator.md、dw-explorer.md、data-quality-checker.md。将 Step 4/5/7 的执行全部下放，预计主 context compact 频率降低 50%~70%。
