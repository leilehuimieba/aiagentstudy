# Miles Project Technical Analysis (2) — Key Technologies

- BestBlogs URL: https://www.bestblogs.dev/article/10a1a93e
- Original publisher URL: https://mp.weixin.qq.com/s?__biz=MzI4OTA3NjQxNA==&mid=2247489357&idx=1&sn=7b73b1ba36f84b136818250458c680e6
- Source: BestBlogs / 罗西的思考
- Publish time: 2026-06-16 20:23:00
...
0x00 概要
   ...
   Miles 的技术特色总结如下：
      ...
      Agentic-First
       ...
       Session Server + TITO + agentic_tool_call → miles/rollout/session/ + miles/rollout/generate_hub/
      ...
      miles_plugins/
        包 + middleware_hub + megatron_to_hf/
...
tokens_B = tokenizer.apply_chat_template(messages)  # 重新 tokenize 全部
   ...
   问题：tokens_B 的前 1000 个 token 不等于tokens_A。
...
pg_loss = pg_loss * tis_weights                           # 修正 policy gradient
   ...
   三步工作流：
     ...
     推理时记录：SGLang 推理时设置 enable_return_routed_experts=True，将每层的 top-k expert indices 记录到 Sample.rollout_routed_experts

    

    
     训练时回放：通过 hook 注册到每个 MoE 层的 topk_fn，用记录的路由替代实时计算（强制回放相同的 routing, 绕过 router 网络）

    

    
     四阶段状态机：fallthrough（正常路由）→ record（记录训练引擎自己的路由，用于 CI 对比）→ replay_forward（重放推理路由，计算 log prob）→ replay_backward（重放推理路由，计算梯度）
   ...
   三层抽象如下：
     ...
     TrueOnPolicyModelProfile：描述模型的 True-On-Policy 能力（支持哪些训练后端、哪些 CP 布局等）

    

    
     TrueOnPolicyContract：定义达成比特级一致所需的策略（attention backend、是否禁用 RoPE fusion、是否 batch-invariant 等）

    

    
     TrueOnPolicyKernelPolicy：将契约参数转化为实际的 CLI 参数和环境变量
...
if args.megatron_to_hf_mode == "bridge":
   ...
   7.4 全部 Bridge 类 (9 个)
      ...
      DeepseekV3Bridge
       ...
       alltoall dispatch, expert bias, shared_head
...
def gc_by_weight_version(self, current_weight_version, gc_threshold_k):
   ...
   8.9 性能特征
       ...
       10000 tokens (总 token_ids 计数，非节点数)
