...
ROOT = Path(r'D:\newwork\aiagentstudy')
RAW_LIST = ROOT / 'knowledge' / 'raw' / 'current-edge-latest-page4.json'
INDEX = ROOT / 'knowledge' / 'catalog' / 'articles-index.md'
MEMORY = ROOT / 'AI_AGENT_MEMORY.md'
REPORT_DIR = ROOT / 'knowledge' / 'raw'

MANUAL = {
    '9aef4fe3': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    '2f8896ed': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    'f195e3e0': ('04-evaluation-guardrails', 'Evaluation, Guardrails, Tools/Actions'),
    '7f56589c': ('01-context-memory', 'Context/State, Memory, Tools/Actions'),
    '380ec148': ('04-evaluation-guardrails', 'Evaluation, Guardrails, Tools/Actions'),
    '53f527ae': ('02-tools-actions', 'Tools/Actions, Control Loop, Deliverable'),
    '407965a3': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    '811c9a29': ('02-tools-actions', 'Tools/Actions, Control Loop, Deliverable'),
    '3f94f358': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    'f7d927a1': ('03-control-loop', 'Goal, Control Loop, Tools/Actions, Evaluation'),
}
...
for item in entries:
    ...
    article_md = (
        ...
        f"- Capture route: article discovered from logged-in Edge/OpenCLI latest page 4 feed; page/content captured from BestBlogs resource APIs\n"
    ...
    source_md = (
        ...
        f"## Evidence Notes\n\n"
        f"- Discovery source: logged-in Edge/OpenCLI session latest list endpoint page 4 (`/api/proxy/resources?page=4&pageSize=10&timeFilter=1w&language=all&sortType=latest&type=ARTICLE&qualifiedFilter=false&uiLang=en`).\n"
    ...
    memory_notes.append(f"On 2026-05-24, captured {bbid}: {title} ({topic}, slug `{slug}`) from the logged-in Edge/OpenCLI latest article page 4 feed, with full text, source evidence, and raw page/content JSON saved.")
    seen.add(norm)
    time.sleep(1)
