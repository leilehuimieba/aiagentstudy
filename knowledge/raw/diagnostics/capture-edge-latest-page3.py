...
ROOT = Path(r'D:\newwork\aiagentstudy')
RAW_LIST = ROOT / 'knowledge' / 'raw' / 'current-edge-latest-page3.json'
INDEX = ROOT / 'knowledge' / 'catalog' / 'articles-index.md'
MEMORY = ROOT / 'AI_AGENT_MEMORY.md'
REPORT_DIR = ROOT / 'knowledge' / 'raw'

MANUAL = {
    '98046252': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    'a2de1250': ('01-context-memory', 'Context/State, Memory, Tools/Actions'),
    '723ff15a': ('04-evaluation-guardrails', 'Evaluation, Guardrails, Tools/Actions'),
    'ad850b1e': ('02-tools-actions', 'Tools/Actions, Control Loop, Deliverable'),
    '78c6e17d': ('02-tools-actions', 'Tools/Actions, Control Loop, Deliverable'),
    '3dcf5158': ('01-context-memory', 'Context/State, Memory, Tools/Actions'),
    '97809db8': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    '259ec643': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    '1d51b31d': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    '85be24fb': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
}
...
for item in entries:
    ...
    article_md = (
        ...
        f"- Capture route: article discovered from logged-in Edge/OpenCLI latest page 3 feed; page/content captured from BestBlogs resource APIs\n"
    ...
    source_md = (
        ...
        f"## Evidence Notes\n\n"
        f"- Discovery source: logged-in Edge/OpenCLI session latest list endpoint page 3 (`/api/proxy/resources?page=3&pageSize=10&timeFilter=1w&language=all&sortType=latest&type=ARTICLE&qualifiedFilter=false&uiLang=en`).\n"
    ...
    memory_notes.append(f"On 2026-05-24, captured {bbid}: {title} ({topic}, slug `{slug}`) from the logged-in Edge/OpenCLI latest article page 3 feed, with full text, source evidence, and raw page/content JSON saved.")
    seen.add(norm)
    time.sleep(1)
