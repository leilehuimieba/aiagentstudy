...
ROOT = Path(r'D:\newwork\aiagentstudy')
RAW_LIST = ROOT / 'knowledge' / 'raw' / 'current-edge-latest-page2.json'
INDEX = ROOT / 'knowledge' / 'catalog' / 'articles-index.md'
MEMORY = ROOT / 'AI_AGENT_MEMORY.md'
REPORT_DIR = ROOT / 'knowledge' / 'raw'

MANUAL = {
    '06fad8fd': ('02-tools-actions', 'Tools/Actions, Control Loop, Deliverable'),
    '0c4caf52': ('01-context-memory', 'Context/State, Memory, Tools/Actions'),
    'a8f90274': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    'fca204e7': ('04-evaluation-guardrails', 'Evaluation, Guardrails, Tools/Actions'),
    'b9260481': ('03-control-loop', 'Goal, Control Loop, Tools/Actions, Evaluation'),
    '638998b1': ('02-tools-actions', 'Tools/Actions, Control Loop, Deliverable'),
    '9bade3d5': ('03-control-loop', 'Goal, Control Loop, Tools/Actions, Evaluation'),
    '98c355f2': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    '8bdb40d5': ('03-control-loop', 'Goal, Control Loop, Tools/Actions, Evaluation'),
    '7aff9979': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
}
...
for item in entries:
    ...
    article_md = (
        ...
        f"- Capture route: article discovered from logged-in Edge/OpenCLI latest page 2 feed; page/content captured from BestBlogs resource APIs\n"
    ...
    source_md = (
        ...
        f"## Evidence Notes\n\n"
        f"- Discovery source: logged-in Edge/OpenCLI session latest list endpoint page 2 (`/api/proxy/resources?page=2&pageSize=10&timeFilter=1w&language=all&sortType=latest&type=ARTICLE&qualifiedFilter=false&uiLang=en`).\n"
    ...
    memory_notes.append(f"On 2026-05-24, captured {bbid}: {title} ({topic}, slug `{slug}`) from the logged-in Edge/OpenCLI latest article page 2 feed, with full text, source evidence, and raw page/content JSON saved.")
    seen.add(norm)
    time.sleep(1)
