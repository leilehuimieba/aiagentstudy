...
ROOT = Path(r'D:\newwork\aiagentstudy')
RAW_LIST = ROOT / 'knowledge' / 'raw' / 'current-edge-latest-page5.json'
INDEX = ROOT / 'knowledge' / 'catalog' / 'articles-index.md'
MEMORY = ROOT / 'AI_AGENT_MEMORY.md'
REPORT_DIR = ROOT / 'knowledge' / 'raw'

MANUAL = {
    'e372ffbd': ('02-tools-actions', 'Tools/Actions, Control Loop, Deliverable'),
    'c5bb1e16': ('04-evaluation-guardrails', 'Evaluation, Guardrails, Tools/Actions'),
    '12cfb9bb': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    '9be68136': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    'd92190a7': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
    '330ed662': ('03-control-loop', 'Goal, Control Loop, Tools/Actions, Evaluation'),
    '0866439a': ('01-context-memory', 'Context/State, Memory, Tools/Actions'),
    '04b41b70': ('03-control-loop', 'Goal, Control Loop, Tools/Actions, Evaluation'),
    'd142a736': ('06-frontier-radar', 'Model, Frontier Radar, Product Workflow'),
}
...
for item in entries:
    ...
    article_md = (
        ...
        f"- Capture route: article discovered from logged-in Edge/OpenCLI latest page 5 feed; page/content captured from BestBlogs resource APIs\n"
    ...
    source_md = (
        ...
        f"## Evidence Notes\n\n"
        f"- Discovery source: logged-in Edge/OpenCLI session latest list endpoint page 5 (`/api/proxy/resources?page=5&pageSize=10&timeFilter=1w&language=all&sortType=latest&type=ARTICLE&qualifiedFilter=false&uiLang=en`).\n"
    ...
    memory_notes.append(f"On 2026-05-24, captured {bbid}: {title} ({topic}, slug `{slug}`) from the logged-in Edge/OpenCLI latest article page 5 feed, with full text, source evidence, and raw page/content JSON saved.")
    seen.add(norm)
